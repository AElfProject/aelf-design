...
import ProgressError from 'assets/progress/error.svg?react'
import ProgressSuccess from 'assets/progress/success.svg?react'
...
const StepItemIcon = ({
  ...
  if (status === 'exception') {
    return errorIcon || <ProgressError className={cx(styles.errorIcon)} />
  }
  if (percent === 100) {
    return successIcon || <ProgressSuccess className={cx(styles.successIcon)} />
  }
  return (
    <Progress
...
const StepItem = ({
  ...
  return (
    <div
      className={cx(styles.progressStepItem, prefixCls + '-progress-step-item')}
    >
      ...
      >
        ...
        <div className={cx(styles.progressStepNum)}>
          <span
            className={cx(
              styles.stepNumText,
              !available && styles.stepNumTextDisabled
            )}
          >
            Step {step}
          </span>
  ...
  return (
    <div
      className={cx(
        ...
        prefixCls + '-progress-steps-container'
      )}
    >
      {stepsData.map((stepItem, index) => {
        return (
          <div
            key={index}
            className={cx(styles.progressSteps, prefixCls + '-progress-steps')}
          >
            <StepItem
              step={index + 1}
